...
router.get("/api/stats", async (req, res) => {
    ...
    `);
        const [totalLiberado] = await pool.query("SELECT COALESCE(SUM(mb_liberados), 0) AS mb_total FROM pcs_historial_limpiezas");
        const [topPcs] = await pool.query(`
...
router.get("/api/stats/modelos", async (req, res) => {
    try {
        const [rows] = await pool.query(`
...
router.get("/api/stats/programas", async (req, res) => {
...
router.get("/api/stats/areas", async (req, res) => {
    try {
        const [rows] = await pool.query(`
      SELECT CASE
...
router.post("/api/programas", async (req, res) => {
    try {
        ...
        const [pc] = await pool.query("SELECT id FROM pcs WHERE serial=?", [serial]);
        if (!pc.length)
...
router.get("/api/metrics", async (req, res) => {
    try {
        ...
        const [[onlineRow]] = await pool.query(`SELECT COUNT(*) AS total FROM pcs WHERE activo=1 AND ultimo_reporte >= DATE_SUB(NOW(), INTERVAL 15 MINUTE)`);
        // Alertas 24h: comandos con estado error en ultimas 24h
        const [[alertsRow]] = await pool.query(`SELECT COUNT(*) AS total FROM pcs_comandos WHERE estado='error' AND creado >= DATE_SUB(NOW(), INTERVAL 24 HOUR)`);
        // Ultimo backup: ultima limpieza registrada
        const [[backupRow]] = await pool.query(`SELECT MAX(fecha) AS ultima FROM pcs_historial_limpiezas`);
        // Tunnel health: fetch a impresoras-ts
        let tunnelOk = false;
        try {
            const http = require('http');
            await new Promise((resolve, reject) => {
                const req2 = http.get('http://localhost:3000/health', (r) => {
...
router.get("/api/ai-score/flota", async (req, res) => {
    try {
        const [rows] = await pool.query(`
      SELECT
    ...
    `);
        const pcs = rows.map(pc => {
            let score = 0;
            const criterios = {};
            criterios.npu = !!pc.tiene_npu;
            if (criterios.npu)
                score += 40;
            criterios.ai_ready = !!pc.es_ai_ready;
            if (criterios.ai_ready)
                score += 20;
            criterios.vpro = !!pc.tiene_vpro;
            if (criterios.vpro)
                score += 10;
            criterios.tpm = !!pc.tiene_tpm && pc.tpm_version?.startsWith('2');
            if (criterios.tpm)
                score += 10;
            criterios.ram = (pc.ram_gb || 0) >= 16;
            if (criterios.ram)
                score += 10;
            criterios.secure_boot = !!pc.secure_boot;
            if (criterios.secure_boot)
                score += 5;
            criterios.disco_ssd = ['SSD', 'NVMe', 'Solid State Drive'].includes(pc.tipo_disco || '');
            if (criterios.disco_ssd)
                score += 5;
            const categoria = score >= 86 ? 'ai_ready' :
                score >= 61 ? 'preparado' :
        ...
        (0, logger_1.logError)("AI_SCORE_ERROR", e.message);
        res.status(500).json({ error: e.message });
    }
});
