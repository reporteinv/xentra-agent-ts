...
function esDriver(nombre, fabricante) {
...
router.get('/api/stats/programas-raros', async (req, res) => {
    try {
        const [[countRow]] = await db_1.default.query('SELECT COUNT(*) as total FROM pcs WHERE activo=1 AND deleted_at IS NULL');
    ...
    `);
        ...
        for (const row of rows) {
            if (esExcluido(row.nombre))
                continue;
            let estado = row.estado_manual;
            if (!row.manual) {
                if (esDriver(row.nombre, row.fabricante || ''))
                    estado = 'driver';
                else if (/^kaspersky/i.test(row.nombre))
                    estado = 'permitido';
                else if (/^7-zip|^winrar|^pdf24|^foxit pdf|^notepad\+\+|^google chrome|^mozilla firefox|^google drive|^vlc|^zoom/i.test(row.nombre))
                    estado = 'permitido';
                else
                    estado = 'sospechoso';
            }
            ...
            if (grupo) {
                if (!grupoMap[grupo.key]) {
                    ...
                    if (row.manual)
                        grupoMap[grupo.key].estado = row.estado_manual;
                }
            }
            else {
                sinGrupo.push({ ...row });
        ...
        const kpis = {
            ...
            drivers: programas.filter((p) => p.estado === 'driver').length,
        };
