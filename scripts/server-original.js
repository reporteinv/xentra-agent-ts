...
function requireAuth(req, res, next) {
  ...
  if (req.path.startsWith('/api/comandos/') && req.method === 'GET') return next();
  if (req.path === '/api/comandos/resultado') return next();
  if (req.method === 'DELETE' && req.path.startsWith('/api/pcs/')) return next();
  if (req.path === '/api/programas') return next();
  if (req.path === '/api/stats/areas') return next();
  if (req.path === '/api/descargar-agente') return next();
  if (req.method === 'GET' && req.path.match(/\/api\/pcs\/[^\/]+\/usb/)) return next();
...
app.post('/api/pcs/:id/observacion', async (req, res) => {
  try {
    ...
    await db.query("UPDATE pcs SET observacion=? WHERE id=?", [observacion, req.params.id]);
    res.json({ ok: true });
...
app.post('/api/reportar', async (req, res) => {
  try {
    ...
    const {
      serial, nombre_equipo, modelo, usuario, ip_local,
    ...
    if (!serial || !nombre_equipo) {
      return res.status(400).json({ error: 'serial y nombre_equipo son requeridos' });
    }
    const [[aliasRow]] = await db.query("SELECT modelo_display FROM modelo_alias WHERE modelo_original=?", [modelo]);
    ...
    if (!aliasRow && modeloFinal) {
      ...
      else if (modeloFinal.includes('HP 240')) modeloFinal = 'HP 240 G7 Notebook PC';
    }
    ...
    await db.query(`
      INSERT INTO pcs 
        ...
        ultimo_reporte = NOW()
    `, [
      serial, nombre_equipo, modeloFinal || null, usuario || null, ip_local || null,
      
      espacio_libre_gb || null, espacio_total_gb || null,
      mb_liberados_ultima != null ? mb_liberados_ultima : null, 
      ultima_limpieza || null,
      ram_gb || null, procesador || null, version_windows || null
      ]);



    const [rows] = await db.query('SELECT id FROM pcs WHERE serial = ?', [serial]);
    ...
    if (cmds.length > 0) {
      comando = 'limpiar';
      await db.query("UPDATE comandos_limpieza SET estado='ejecutado', fecha_ejecucion=NOW() WHERE id=?", [cmds[0].id]);
    }
...
app.get('/api/pcs/:id', async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT * FROM pcs WHERE id = ?', [req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: 'PC no encontrado' });
    res.json(rows[0]);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/limpiar-masivo', async (req, res) => {
  try {
    const [pcsActivos] = await db.query("SELECT id FROM pcs WHERE activo=1 AND ultimo_reporte >= DATE_SUB(NOW(), INTERVAL 30 DAY)");
    if (pcsActivos.length === 0) return res.status(400).json({ error: 'No hay PCs activos' });
    const [camp] = await db.query("INSERT INTO campanas_limpieza (total_pcs) VALUES (?)", [pcsActivos.length]);
    const campanaId = camp.insertId;
    const valores = pcsActivos.map(p => [campanaId, p.id]);
    await db.query("INSERT INTO comandos_limpieza (campana_id, pc_id) VALUES ?", [valores]);
...
app.get('/api/limpieza/detalle/:id', async (req, res) => {
  try {
    await db.query("UPDATE comandos_limpieza SET estado='expirado' WHERE estado='pendiente' AND fecha_expiracion <= NOW()");
    const [rows] = await db.query(`
...
app.get('/api/export/excel', async (req, res) => {
  try {
    ...
    rows.forEach(r => ws.addRow({
      ...
      ip_local: r.ip_local,
      espacio_libre_gb: r.espacio_libre_gb ? Math.round(r.espacio_libre_gb) : null,
      espacio_total_gb: r.espacio_total_gb ? Math.round(r.espacio_total_gb) : null,
      mb_liberados_ultima: r.mb_liberados_ultima != null ? parseFloat((r.mb_liberados_ultima / 1024).toFixed(1)) : null,
      ultima_limpieza: r.ultima_limpieza ? new Date(r.ultima_limpieza).toLocaleDateString("es-CO") : null,
...
app.get('/api/export/pdf', async (req, res) => {
  try {
    ...
    try { doc.image(logoPath, 760, 5, { width: 65, height: 65 }); } catch(e) { console.log('Logo err:', e.message); }

    // Título
    doc.fontSize(18).fillColor('#2c3e50').text('Reporte', { align: 'center' });
    doc.fontSize(10).fillColor('#7f8c8d')
       .text(`Generado: ${new Date().toLocaleString('es-CO')} | Total: ${rows.length} PCs`,
             { align: 'center' });
    doc.moveDown(1);

    // Encabezado tabla
    const headers = ['Estado', 'Serial', 'Modelo', 'Procesador', 'RAM', 'Windows', 'Usuario', 'IP', 'Disco libre', 'GB liber.', 'Últ. limpieza'];
    const widths  = [42, 75, 95, 112, 38, 48, 80, 62, 72, 48, 80];
    let y = doc.y;
    let x = 30;

    doc.fontSize(9).fillColor('#ffffff');
    doc.rect(30, y, widths.reduce((a, b) => a + b, 0), 20).fill('#34495e');
    ...
    rows.forEach((r, idx) => {
      if (y > 520) {
        doc.addPage();
        try { doc.image(logoPath, 760, 5, { width: 65, height: 65 }); } catch(e) {}
        // Encabezado en nueva página
        doc.fontSize(18).fillColor('#2c3e50').text('Reporte', { align: 'center' });
        doc.fontSize(10).fillColor('#7f8c8d').text(`Generado: ${new Date().toLocaleString('es-CO')} | Total: ${rows.length} PCs`, { align: 'center' });
        doc.moveDown(0.5);
        // Repetir header tabla
        let xh = 30;
        const ny = doc.y;
        doc.rect(30, ny, widths.reduce((a, b) => a + b, 0), 20).fill('#34495e');
        doc.fontSize(9).fillColor('#ffffff');
        headers.forEach((h, i) => { doc.text(h, xh, ny + 7, { width: widths[i], align: 'center' }); xh += widths[i]; });
        y = ny + 20;
        doc.fontSize(8);
      }
      ...
      // Fondo alternado
      if (idx % 2 === 0) {
        doc.rect(30, y, widths.reduce((a, b) => a + b, 0), 18).fill('#f9fafb');
      }
      ...
      const cells = [
        ...
        { text: r.ip_local || '-' },
        { text: r.espacio_libre_gb ? `BARRA:${r.espacio_libre_gb}:${r.espacio_total_gb}` : '-' },
        { text: r.mb_liberados_ultima ? `${(r.mb_liberados_ultima / 1024).toFixed(1)} GB` : '-' },
        { text: r.ultima_limpieza ? new Date(r.ultima_limpieza).toLocaleString('es-CO', { dateStyle: 'short', timeStyle: 'short' }) : '-' }
      ];
      ...
      cells.forEach((c, i) => {
        if (c.text && c.text.startsWith('BARRA:')) {
          ...
          doc.rect(x + 4, barY, Math.round(barW * pct / 100), barH).fill(color);
          // texto
          doc.fillColor('#2c3e50').fontSize(7);
          doc.text(`${pct}%`, x, barY + 8, { width: widths[i], align: 'center' });
          doc.fontSize(8);
        } else {
          doc.fillColor(c.color || '#2c3e50');
          doc.text(c.text, x, y + 7, { width: widths[i], align: 'center', ellipsis: true });
        }
        x += widths[i];
      });
    ...
    // ============================================
    doc.addPage();
    doc.fontSize(16).fillColor('#2c3e50').text('Top PCs — GB Liberados Acumulados', 30, doc.y, { width: 752, align: 'center' });
    doc.moveDown(0.5);
    doc.fontSize(9).fillColor('#7f8c8d').text(`Generado: ${new Date().toLocaleString('es-CO')}`, { align: 'center' });
    doc.moveDown(1);
    ...
    if (topPcs.length > 0) {
      ...
      topPcs.forEach((pc, idx) => {
        ...
        doc.text(pc.usuario ? pc.usuario.split('\\').pop() : (pc.serial || '-'), chartX - 90, chartY + 6, { width: 85, align: 'right' });
        ...
        if (bW > 40) {
          doc.text(`${gb} GB`, chartX + bW - 40, chartY + 7, { width: 38, align: 'right' });
        } else {
      ...
      doc.fontSize(12).fillColor('#999999').text('Sin datos de limpiezas registradas', { align: 'center' });
    }
    ...
    if (topProgramas.length > 0) {
      doc.addPage();
      try { doc.image(logoPath, 760, 5, { width: 65, height: 65 }); } catch(e) {}
      doc.fontSize(16).fillColor('#2c3e50').text('Top 10 — Programas más instalados', 30, doc.y, { width: 752, align: 'center' });
      doc.moveDown(0.5);
      doc.fontSize(9).fillColor('#7f8c8d').text(`Generado: ${new Date().toLocaleString('es-CO')}`, { align: 'center' });
      doc.moveDown(1);
      ...
      topProgramas.forEach((p) => {
        ...
        const bW = Math.round(chartW * pct);
        const nombre = p.nombre.length > 38 ? p.nombre.substring(0, 38) + '…' : p.nombre;
        doc.fontSize(8).fillColor('#2c3e50');
        doc.text(nombre, chartX - 240, chartY + 5, { width: 235, align: 'right' });
        doc.rect(chartX, chartY, chartW, barH).fill('#ecf0f1');
        doc.rect(chartX, chartY, bW, barH).fill('#3498db');
        doc.fontSize(8).fillColor('#ffffff');
        if (bW > 30) {
          doc.text(`${p.total_pcs} PCs`, chartX + bW - 40, chartY + 6, { width: 38, align: 'right' });
        } else {
          doc.fillColor('#2c3e50').text(`${p.total_pcs} PCs`, chartX + bW + 4, chartY + 6);
        }
        chartY += barH + 10;
      });
    }
...
app.post('/api/comandos/crear', async (req, res) => {
  try {
    ...
    await db.query('UPDATE comandos SET estado = "cancelado" WHERE pc_id = ? AND estado = "pendiente"', [pc_id]);
    await db.query('INSERT INTO comandos (pc_id, comando, estado) VALUES (?, "limpiar", "pendiente")', [pc_id]);
...
app.get('/api/comandos/:serial', async (req, res) => {
  try {
    ...
    const [pcs] = await db.query('SELECT id FROM pcs WHERE serial = ?', [req.params.serial]);
    if (!pcs.length) return res.json({ hay: false });
    const [cmds] = await db.query('SELECT id, comando FROM comandos WHERE pc_id = ? AND estado = "pendiente" ORDER BY creado ASC LIMIT 1', [pcs[0].id]);
    if (!cmds.length) return res.json({ hay: false });
    await db.query('UPDATE comandos SET estado = "ejecutando" WHERE id = ?', [cmds[0].id]);
...
app.post('/api/comandos/resultado', async (req, res) => {
  try {
    ...
    await db.query('UPDATE comandos SET estado = ?, ejecutado = NOW(), resultado = ? WHERE id = ?', [estado || 'completado', mb_liberados != null ? mb_liberados + ' MB liberados' : null, id]);
    if (mb_liberados != null && espacio_libre_gb != null) {
      await db.query('UPDATE pcs SET mb_liberados_ultima = ?, espacio_libre_gb = ?, ultima_limpieza = NOW(), ultimo_reporte = NOW() WHERE id = (SELECT pc_id FROM comandos WHERE id = ?)', [mb_liberados, espacio_libre_gb, id]);
    }
    res.json({ ok: true });
  } catch (err) {
...
app.get('/api/comandos/estado/:pc_id', async (req, res) => {
  try {
    if (!req.session.autenticado) return res.status(401).json({ error: 'No autenticado' });
    const [rows] = await db.query('SELECT id, estado, creado, ejecutado, resultado FROM comandos WHERE pc_id = ? ORDER BY creado DESC LIMIT 1', [req.params.pc_id]);
...
app.get('/api/export/historial-excel', async (req, res) => {
  try {
    ...
    let where = [];
    let params = [];
    if (pc_id) { where.push('h.pc_id = ?'); params.push(pc_id); }
    if (desde) { where.push('h.fecha >= ?'); params.push(desde); }
    if (hasta) { where.push('h.fecha <= ?'); params.push(hasta + ' 23:59:59'); }
    ...
    rows.forEach(r => ws.addRow({
      ...
      mb_liberados: r.mb_liberados != null ? parseFloat((r.mb_liberados / 1024).toFixed(2)) : null,
      espacio_libre_gb: r.espacio_libre_gb ? Math.round(r.espacio_libre_gb) : null
    }));

    const fecha = new Date().toISOString().slice(0, 10);
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', 'attachment; filename="historial_' + (pc_id && rows[0]?.usuario ? rows[0].usuario.replace(/.*\\/,'').replace(/[^a-zA-Z0-9]/g,'_') + '_' : '' ) + fecha + '.xlsx"');
    await wb.xlsx.write(res);
    res.end();
  } catch (err) {
...
app.get('/api/export/historial-pdf', async (req, res) => {
  try {
    ...
    const fecha = new Date().toISOString().slice(0, 10);
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename="historial_' + (pc_id && rows[0]?.usuario ? rows[0].usuario.replace(/.*\\/,'').replace(/[^a-zA-Z0-9]/g,'_') + '_' : '' ) + fecha + '.pdf"');
    ...
    doc.fontSize(18).fillColor('#2c3e50').text('Historial de Limpiezas', titleX, 30, { align: 'center' });
    doc.fontSize(10).fillColor('#7f8c8d')
       .text(`Generado: ${new Date().toLocaleString('es-CO')} | Total: ${rows.length} registros`, { align: 'center' });
    doc.moveDown(1);

    const headers = ['Serial', 'Modelo', 'Usuario', 'Fecha', 'GB Liberados', 'Disco Libre'];
    ...
    function drawHeader(y) {
      ...
      doc.fontSize(9).fillColor('#ffffff');
      headers.forEach((h, i) => {
        doc.text(h, x + 4, y + 6, { width: widths[i] - 8, align: 'left', lineBreak: false });
        x += widths[i];
      });
      return y + 20;
    }
    ...
    rows.forEach((r, idx) => {
      ...
      const cells = [
        ...
        r.mb_liberados != null ? (r.mb_liberados / 1024).toFixed(2) + ' GB' : '-',
        r.espacio_libre_gb ? parseFloat(r.espacio_libre_gb).toFixed(0) + ' GB' : '-'
      ];
      cells.forEach((c, i) => {
        doc.fillColor('#2c3e50');
        doc.text(c, x + 4, y + 5, { width: widths[i] - 8, align: 'left', lineBreak: false, ellipsis: true });
        x += widths[i];
      });
      y += rowH;
    });
...
app.get('/api/export/programas-pdf', async (req, res) => {
  try {
    ...
    const fecha = new Date().toISOString().slice(0, 10);
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename="programas_' + fecha + '.pdf"');
    const doc = new PDFDocument({ size: 'A4', layout: 'landscape', margin: 30 });
    doc.pipe(res);
    // Logo UNGRD
    const logoPath = __dirname + '/public/assets/logo-ungrd.png';
    try { doc.image(logoPath, 750, 5, { width: 80, height: 50 }); } catch(e) {}

    doc.fontSize(18).fillColor('#2c3e50').text('Inventario de Programas', { align: 'center' });
    doc.fontSize(10).fillColor('#7f8c8d')
       .text(`Generado: ${new Date().toLocaleString('es-CO')} | Total: ${rows.length} registros`, { align: 'center' });
    doc.moveDown(1);

    const headers = ['Serial', 'Modelo', 'Usuario', 'Programa', 'Version', 'Fabricante'];
    ...
    rows.forEach((r, idx) => {
      if (y > 520) {
        ...
        try { doc.image(logoPath, 750, 5, { width: 80, height: 50 }); } catch(e) {}
        doc.fontSize(18).fillColor('#2c3e50').text('Inventario de Programas', { align: 'center' });
        doc.fontSize(10).fillColor('#7f8c8d').text(`Generado: ${new Date().toLocaleString('es-CO')} | Total: ${rows.length} registros`, { align: 'center' });
        doc.moveDown(0.5);
        y = doc.y;
        y = drawHeader(y);
        doc.fontSize(8);
      ...
      cells.forEach((c, i) => {
        doc.fillColor('#2c3e50');
        doc.text(c, x + 4, y + 4, { width: widths[i] - 8, align: 'left', lineBreak: false, ellipsis: true });
        x += widths[i];
      });
      y += rowH;
    });
...
app.delete('/api/pcs/:serial', async (req, res) => {
  try {
    ...
    const [result] = await db.query('DELETE FROM pcs WHERE serial = ?', [req.params.serial]);
    if (result.affectedRows === 0) return res.status(404).json({ error: 'PC no encontrado' });
...
app.post('/api/pcs/:serial/usb', async (req, res) => {
  try {
    ...
    if (bloqueado === undefined) return res.status(400).json({ error: 'bloqueado requerido' });
...
app.get('/api/pcs/:serial/usb', async (req, res) => {
  try {
    const token = req.headers['x-agent-token'];
    if (token !== process.env.AGENT_TOKEN) return res.status(401).json({ error: 'Token invalido' });
    const [rows] = await db.query('SELECT usb_bloqueado FROM pcs WHERE serial = ?', [req.params.serial]);
...
app.post('/api/programas', async (req, res) => {
  try {
    ...
    const [pc] = await db.query('SELECT id FROM pcs WHERE serial = ?', [serial]);
    if (!pc.length) return res.status(404).json({ error: 'PC no encontrado' });
    const pc_id = pc[0].id;
    await db.query('DELETE FROM programas WHERE pc_id = ?', [pc_id]);
    if (programas.length > 0) {
      const values = programas.map(p => [pc_id, p.nombre, p.version || null, p.fabricante || null]);
      await db.query('INSERT INTO programas (pc_id, nombre, version, fabricante) VALUES ?', [values]);
    }
...
app.get('/api/export/programas-excel', async (req, res) => {
  try {
    ...
    rows.forEach((r, idx) => {
      ...
      row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: idx % 2 === 0 ? 'FFEEF2F7' : 'FFFFFFFF' } };
    ...
    const fecha = new Date().toISOString().slice(0, 10);
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="programas_${fecha}.xlsx"`);
    await wb.xlsx.write(res);
    res.end();
  } catch (err) {
...
app.get('/health', (req, res) => res.json({ ok: true, service: 'xentra-agent' }));
