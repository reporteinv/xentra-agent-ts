...
router.post('/api/pcs/:id/observacion', async (req: Request, res: Response) => {
  try {
    ...
    await pool.query("UPDATE pcs SET observacion=? WHERE id=?", [observacion, req.params.id]);
    res.json({ ok: true });
...
router.get('/api/pcs/colores', async (req: Request, res: Response) => {
  try {
    const [rows] = await pool.query<RowDataPacket[]>("SELECT pc_id, color FROM colores_pc");
    const map: any = {};
...
router.post('/api/reportar', async (req: Request, res: Response) => {
  try {
    ...
    const { serial, nombre_equipo, modelo, usuario, ip_local, espacio_libre_gb, espacio_total_gb,
      mb_liberados_ultima, ultima_limpieza, ram_gb, procesador, version_windows } = req.body;
    if (!serial || !nombre_equipo) return res.status(400).json({ error: 'serial y nombre_equipo son requeridos' });
    const [[aliasRow]] = await pool.query<RowDataPacket[]>("SELECT modelo_display FROM modelo_alias WHERE modelo_original=?", [modelo]) as any;
    ...
    `, [serial, nombre_equipo, modeloFinal||null, usuario||null, ip_local||null,
        ...
        ultima_limpieza||null, ram_gb||null, procesador||null, version_windows||null]);
    const [rows] = await pool.query<RowDataPacket[]>('SELECT id FROM pcs WHERE serial=?', [serial]);
    const pcId = (rows as any[])[0]?.id;
    if (pcId && mb_liberados_ultima != null) {
      await pool.query('INSERT INTO historial_limpiezas (pc_id, mb_liberados, espacio_libre_gb) VALUES (?,?,?)',
        [pcId, mb_liberados_ultima, espacio_libre_gb]);
    }
    let comando = null;
    const [cmds] = await pool.query<RowDataPacket[]>(
      "SELECT id FROM comandos_limpieza WHERE pc_id=? AND estado='pendiente' AND fecha_expiracion > NOW() LIMIT 1", [pcId]);
    if ((cmds as any[]).length > 0) {
      comando = 'limpiar';
      await pool.query("UPDATE comandos_limpieza SET estado='ejecutado', fecha_ejecucion=NOW() WHERE id=?", [(cmds as any[])[0].id]);
    }
    res.json({ ok: true, mensaje: 'Reporte recibido', comando });
  } catch(err: any) { console.error('[Agent] Error:', err); res.status(500).json({ error: 'Error interno' }); }
});
...
router.get('/api/pcs', async (req: Request, res: Response) => {
  try {
    ...
    `);
    res.json(rows);
  } catch(err: any) { res.status(500).json({ error: 'Error consultando PCs' }); }
});
...
router.get('/descargar/xentra-agent.ps1', (req: Request, res: Response) => {
  ...
  res.download(archivo, 'xentra-agent.ps1', (err: any) => { if (err) res.status(500).send('No disponible'); });
});
...
router.get('/api/descargar-agente', (req: Request, res: Response) => {
  ...
  if (!fs.existsSync(ps1) || !fs.existsSync(bat)) return res.status(500).json({ error: 'Archivos no disponibles' });
...
router.get('/api/pcs/:id', async (req: Request, res: Response) => {
  try {
    const [rows] = await pool.query<RowDataPacket[]>('SELECT * FROM pcs WHERE id=?', [req.params.id]);
    if (!(rows as any[]).length) return res.status(404).json({ error: 'PC no encontrado' });
...
router.delete('/api/pcs/:serial', async (req: Request, res: Response) => {
  try {
    await pool.query('UPDATE pcs SET activo=0 WHERE serial=?', [req.params.serial]);
    res.json({ ok: true });
  } catch(e: any) { res.status(500).json({ error: e.message }); }
});
...
router.get('/api/pcs/:serial/usb', async (req: Request, res: Response) => {
  try {
    const [rows] = await pool.query<RowDataPacket[]>('SELECT usb_bloqueado FROM pcs WHERE serial=?', [req.params.serial]);
    res.json((rows as any[])[0] || { usb_bloqueado: 0 });
  } catch(e: any) { res.status(500).json({ error: e.message }); }
});

router.get('/api/pcs/:serial/usb-estado', async (req: Request, res: Response) => {
  try {
    const [rows] = await pool.query<RowDataPacket[]>('SELECT usb_bloqueado FROM pcs WHERE serial=?', [req.params.serial]);
    res.json((rows as any[])[0] || { usb_bloqueado: 0 });
  } catch(e: any) { res.status(500).json({ error: e.message }); }
...
router.post('/api/pcs/:serial/usb', async (req: Request, res: Response) => {
  try {
    ...
    await pool.query('UPDATE pcs SET usb_bloqueado=? WHERE serial=?', [bloquear ? 1 : 0, req.params.serial]);
    res.json({ ok: true });
  } catch(e: any) { res.status(500).json({ error: e.message }); }
});
