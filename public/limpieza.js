...
async function cargarCampanas() {
  try {
    ...
    if (campanas.length === 0) {
      grid.innerHTML =
        '<p style="color:#888;font-size:0.85rem;">No hay campañas aún.</p>';
      return;
    }
...
async function verDetalle(id) {
  ...
  try {
    ...
    document.getElementById("lm-prog-txt").textContent =
      `${ejec} / ${total} (${pctEjec}%)`;

    document.getElementById("lm-subtitulo").textContent =
      `Campaña del ${new Date(rows[0]?.fecha_creacion || Date.now()).toLocaleString("es-CO")}`;
...
function filtrarTabla() {
  ...
  const filtrado = detalleActual.filter(
    (r) =>
...
async function iniciarLimpieza() {
  ...
  try {
    ...
    document.getElementById("btnLimpiarMasivo").textContent =
      "⚡ Nueva limpieza masiva";
  }
}

cargarCampanas();
setInterval(() => {
  if (campanaActiva) verDetalle(campanaActiva);
}, 30000);
