async function cargarDashboard() {
  try {
    ...
    document.getElementById('kpiMB').textContent = formatMB(data.mb_total_liberado);

    // Top 10 PCs (barras horizontales)
    new Chart(document.getElementById('chartTopPcs'), {
      type: 'bar',
      data: {
        labels: data.top_pcs.map(p => p.usuario ? p.usuario.split('\\').pop() : p.nombre_equipo),
        datasets: [{
          label: 'GB liberados',
          data: data.top_pcs.map(p => parseFloat((p.total_liberado / 1024).toFixed(2))),
    ...
    new Chart(document.getElementById('chartPorDia'), {
      ...
      data: {
        ...
        datasets: [
          ...
          {
            label: 'GB liberados',
            data: data.por_dia.map(d => parseFloat((d.mb_dia / 1024).toFixed(2))),
            borderColor: '#9b59b6',
...
async function cargarStatsProgramas() {
  try {
    ...
    const data = await res.json();
    document.getElementById('kpi-total-prog').textContent = data.totales.total_registros.toLocaleString('es-CO');
    document.getElementById('kpi-unicos-prog').textContent = data.totales.total_unicos.toLocaleString('es-CO');
    document.getElementById('kpi-pcs-prog').textContent = data.totales.total_pcs;
    const labels = data.top10.map(p => p.nombre.length > 30 ? p.nombre.substring(0,30)+'…' : p.nombre);
    const valores = data.top10.map(p => p.total_pcs);
    if (window.chartProgramas && typeof window.chartProgramas.destroy === 'function') window.chartProgramas.destroy();
    const canvas = document.getElementById('chartProgramas');
    Chart.getChart(canvas)?.destroy();
    window.chartProgramas = new Chart(document.getElementById('chartProgramas'), {
      type: 'bar',
      data: {
        labels,
        datasets: [{ label: 'PCs', data: valores, backgroundColor: '#3498db', borderRadius: 4 }]
      },
      options: {
        indexAxis: 'y',
        responsive: true,
        maintainAspectRatio: false,
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ` ${ctx.raw} PCs` } } },
        scales: {
          x: { grid: { color: 'rgba(128,128,128,0.1)' }, ticks: { color: '#888', stepSize: 1 } },
          y: { grid: { display: false }, ticks: { color: '#555', font: { size: 11 } } }
        }
      }
    });
  } catch(e) { console.error('Error stats programas:', e); }
}
...
async function cargarGraficaAreas() {
  try {
    ...
    const areasValidas = ['Apoyo','Juridico','Subdireccion','GTI','Admin','SecGeneral',
      'Planeacion','Reduccion','Financi','GRC','CDI','Conocimiento','Simex','Control','Comunicaciones'];

    const filtrado = data.map(d => ({
    ...
    const colores = [
      '#2952a3','#4a90d9','#e74c3c','#2ecc71','#f39c12',
      '#9b59b6','#1abc9c','#e67e22','#34495e','#e91e63',
      '#00bcd4','#8bc34a','#ff5722','#607d8b','#795548','#ff9800','#aaa'
    ];
...
async function cargarGraficaModelos() {
  try {
    ...
    kpiDiv.innerHTML = datos.map((d, i) => `
    ...
    // Gráfica dona
    if (window.chartModelos && typeof window.chartModelos.destroy === 'function') window.chartModelos.destroy();
